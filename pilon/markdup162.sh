...
/path/to/bin/ibm-java-80-8.0.5.1/bin/java -Xmx180G -jar /path/to/bin/picard-2.18.4/picard.jar MarkDuplicates\
	INPUT=${sample}-arrow.scaffold.rename.162PE.bwamem.bam\
	OUTPUT=${sample}-arrow.scaffold.rename.162PE.bwamem.markdup.bam\
	METRICS_FILE=${sample}-arrow.scaffold.rename.162PE.bwamem.bam.markdup_metrics\
	TMP_DIR=${sample}-arrow.scaffold.rename.162PE.bwamem.markdup.bam.tmp\
	COMPRESSION_LEVEL=6\
	CREATE_INDEX=true\
	ASSUME_SORTED=true\
