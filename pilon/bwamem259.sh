...
/path/to/bwa/0.7.17/bwa mem\
	-t 22\
	-K 1000000\
	-R '@RG\tID:${sample}-259PE\tPT:ILLUMINA\tPU:259PE\tB:ToMMo:\tSM:${sample}'\
	${sample}-arrow.scaffold.rename\
	${READ1}\
	${READ2}\
| java -jar /path/to/bin/picard-2.18.4/picard.jar SortSam\
	INPUT=/dev/stdin\
	OUTPUT=${sample}-arrow.scaffold.rename.259PE.bwamem.bam\
	TMP_DIR=${sample}-arrow.scaffold.rename.259PE.bwamem.bam.tmp\
